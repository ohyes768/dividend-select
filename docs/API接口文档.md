...
**说明**: 批量获取股息率>=5的股票的120日均线数据，同时返回最新收盘价和偏离度。适用于 n8n 定时调用。

**查询参数**:

| 参数 | 类型 | 必填 | 说明 | 默认值 |
|------|------|------|------|--------|
| `min_yield` | float | 否 | 最小股息率(%) | 5.0 |
| `sort_by` | string | 否 | 排序字段 | `avg_yield_3y` |
| `sort_order` | string | 否 | 排序方向(asc/desc) | `desc` |
...
**说明**: 刷新所有股息率>=5的股票的120日均线数据。该接口耗时较长，建议在非高峰期调用。
...
| `M120数据.csv` | 股息率>=5的股票的120日均线数据（由 API 刷新） |
